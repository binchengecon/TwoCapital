...
with open("./data/PostJump/res-11-13-45", "rb") as f:
...
while  FC_Err > tol and epoch < max_iter:
    ...
    if epoch == 0:
        ...
        # i_d = (1 / 8 - np.sqrt(0.4752) / 8) * np.ones(K_mat.shape)
        # i_g = (1 / 8 - np.sqrt(0.4752) / 8) * np.ones(K_mat.shape)
        # i_d = 0.1 * np.ones(K_mat.shape)
        # i_g = 0.1 * np.ones(K_mat.shape)
        ...
        # i_g[i_g > 1 / phi_g - 1e-15] = 1/phi_g - 1e-15
        # i_g[i_g <  1e-15] =  1e-15
        # i_g = np.zeros(X_mat.shape)
        temp = (A_d - i_d ) * (1 - R_mat) + (A_g - i_g) * R_mat - delta  / ( np.exp(K_mat) *  varphi * dLam) 
        # temp[ temp < 1e-15 ] = 1e-15
        # temp[ temp > A_d - 1e-15] = A_d - 1e-15
        i_l =  temp
        # i_l[i_l > 1  - 1e-15] = 1 - 1e-15
        # i_l = np.zeros(X_mat.shape)
        # i_l = 1e-5 * np.ones(X_mat.shape)
        print(np.min(np.exp(K_mat) * varphi * dLam), np.max(np.exp(K_mat) * varphi * dLam))
     ...
     # updating controls
        ...
        # while Converged == 0 and num < 1000:
            ...
            # i_l_1  = np.zeros(X_mat.shape)
            # # i_l_1 = (A_d - i_d) * (1 - R_mat) + (A_g - i_g) * R_mat - delta / q
            # # i_l_1 /= Lambda_mat
            # # i_l_1[i_l_1 > 1-1e-15] = 1 - 1e-15
            # # i_l_1[i_l_1 < 1e-15] = 1e-15
            ...
            # else:
                ...
                # temp =  (A_d - i_d) * (1 - R_mat) + (A_g - i_g) * R_mat - i_l
                # # temp[temp < 1e-15] = 1e-15
                # # q = delta * ( (A_d - i_d) * (1 - R_mat) + (A_g - i_g) * R_mat - i_l * Lambda_mat ) ** (-1) * fraction + (1 - fraction) * q
                # q = delta * temp**(-1) * fraction + q * (1 - fraction)
            # num += 1
    ...
    if epoch == 0:
        ...
        C_11 = 0.5 * (sigma_d**2 * (1 - R_mat)**2 + sigma_g**2 * R_mat**2)
        C_22 = 0.5 * R_mat**2 * (1 - R_mat)**2 * (sigma_d**2 + sigma_g**2)
    ...
    mu_d = alpha_d + i_d - 0.5 * phi_d * i_d**2 - 0.5 * sigma_d**2
    mu_g = alpha_g + i_g - 0.5 * phi_g * i_g**2 - 0.5 * sigma_g**2
    B_1 = mu_d * (1 - R_mat) + mu_g * R_mat - C_11
    B_2 = (mu_g - mu_d) * R_mat * (1 - R_mat)
    # B_3 = beta_f * e * D_mat
    B_4 =  varphi * i_l * np.exp(K_mat)  - alpha_l * Lambda_mat
    # B_4  = np.zeros(X_mat.shape)
    # B_4 = 0.0001 * np.ones(X_mat.shape)
    ...
    D = delta * np.log(consumption) + delta * K_mat- (gamma_1 + gamma_2 * Temp_mat + gamma_3 * (Temp_mat - 2) * (Temp_mat > 2) )* beta_f * eta * A_d * np.exp(K_mat) * (1 - R_mat)  - 0.5 * (gamma_2 + gamma_3 * (Temp_mat > 2)) * (varsigma * eta * A_d * np.exp(K_mat) * (1 - R_mat) )**2 + Lambda_mat * V_post
