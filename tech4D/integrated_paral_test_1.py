...
test_code = "_epsfrac_"+ str(epsilon)[0:len(str(epsilon))]+"_Paral"+"_Attemp_" +str(Attemp)+"_test_1"
...
def obj_idig(x, RTemp,dRTemp,dKTemp):
    delta = 0.01
    A_d = 0.12
    A_g = 0.15
    phi_d = 8.
    ...
    igTemp=x[1]
    Temp = 0
    Temp += (A_d-idTemp)*(1-RTemp)+ (A_g-igTemp)*RTemp
    Temp = delta*np.log(Temp)
    Temp += (idTemp-phi_d/2*idTemp**2)*(1-RTemp)*(dKTemp-RTemp*dRTemp)
    Temp += (igTemp-phi_g/2*igTemp**2)*RTemp*(dKTemp+(1-RTemp)*dRTemp) 
    return -Temp

from scipy.optimize import minimize 

bounds_pervar = (0,1)
bounds_total =(bounds_pervar,bounds_pervar)
minimize(obj_idig, (0,0), args=(.8,1,1) ,method='SLSQP', bounds= bounds_total)

def sol_obj_idig(RTemp,dRTemp,dKTemp):

def obj_idig_vec(R_1d,dK_1d,dR_1d):
